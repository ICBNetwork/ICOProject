...
contract ICB_ICO is ReentrancyGuard, Ownable {
    ...
    event ConfigSale(SaleType setSaletype, uint256 salePriceInDollar, uint256 everyDayIncreasePrice, uint256 saleStart, uint256 saleEnd, uint8 lockMonths, uint8 vestingMonths);
    ...
    constructor(address funderWallet, address[] memory allowedTokens, bool isNativeAllowed, address aggregator, uint8 tokenDecimals, uint256 saleStart, uint256 saleEnd) Ownable(msg.sender) validAddress(funderWallet) validAddress(aggregator) inputNumberCheck(tokenDecimals) {
    ...
    function addUserByAdmin(address[] memory userAddress, uint256[] memory packageAmount, uint256[] memory userIcbAmount, uint256[] memory icbInDollar, uint256[] memory investTime, uint256[] memory lockMonthTime, uint256[] memory linearVestingTime, string[] memory currentSaleTypes) external onlyOwner returns(bool) {
    ...
    function estimateFund(uint256 packageAmount, BuyType buyType) public view inputNumberCheck(packageAmount) isSalePause returns(uint256, uint256){
        ...
        if(BuyType.eth == buyType){
            ...
            uint256 tokenAmount = packageAmount * (10**_icoConfig.tokenDecimal);
            uint256 icbAmount = (packageAmount * 10**18) / icbInDollarSaleWise;
        ...
        returns (bool)
    {
        ...
        IERC20(tokenAddress).transferFrom(msg.sender, _icoConfig.fundingWallet, estimatedToken);
        ...
        returns (bool)
    {
        ...
        emit BuyWithToken(msg.sender, estimatedNative, icbAmount, currentSaleType);
        return true;
    }
